 ...
 *
 * The login runs in a **fresh browser context**. Cookies ignore ports, so the
...
test("an owner can sign in to the shift app with an emailed passcode", async ({
  ...
  try {
    await test.step("2. request a passcode from the shift login", async () => {
      const shift = await freshContext.newPage();
      ...
      await test.step("3. enter it and land back on the shift origin", async () => {
        ...
        // `app: "shift"` is what aims the session back here; without it the
        // API resolves the admin origin and this wait times out.
